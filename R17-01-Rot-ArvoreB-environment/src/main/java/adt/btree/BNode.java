...
public class BNode<T extends Comparable<T>> {
	...
	public BNode<T> getFirstHalf() {
		BNode<T> left = new BNode<>(this.order);
		
		int aux = (order/2);
		
		for (int i = 0; i < aux; i++) {
			left.addElement(this.getElementAt(i));
			
			if (!this.isLeaf()) {
				left.addChild(i, this.getChildren().get(i));
			}
		}
		
		if (!this.isLeaf()) {
			left.addChild(aux, this.getChildren().get(aux));
		}
		
		return left;
	}
	
	public BNode<T> getSecondHalf() {
		BNode<T> right = new BNode<>(this.order);
		
		int aux = (this.order/2) + 1;
		
		for (int i = aux; i < order; i++) {
			right.addElement(this.getElementAt(i));
			
			if (!this.isLeaf()) {
				right.addChild(i - aux, this.getChildren().get(i));
			}
		}
		
		if (!this.isLeaf()) {
			right.addChild((this.order) - aux, this.getChildren().get(this.order));
		}
